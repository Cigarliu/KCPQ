...
type Client struct {
	...
	connectionLost       atomic.Bool // 是否已进入断线状态
...
// 支持超时、取消等 context 控制
func ConnectWithContext(ctx context.Context, addr string, aes256Key []byte) (*Client, error) {
	// 检查 context 是否已取消
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	...
	client := &Client{
		conn:                 conn,
		aes256Key:            append([]byte(nil), aes256Key...),
		ctx:                  clientCtx,
		cancel:               cancel,
	...
	// 启动连接监控（修复 BUG：检测 receiveLoop 和 heartbeat 退出，触发重连）
...
func Connect(addr string, aes256Key []byte) (*Client, error) {
	return ConnectWithContext(context.Background(), addr, aes256Key)
}
...
func (c *Client) handleMessage(msg *protocol.Message) {
	...
	case protocol.CmdMsg:
		...
		for _, sub := range c.subscriptions {
			if sub.active.Load() && matchSubject(sub.subject, msg.Subject) {
				message := &Message{
					Subject: msg.Subject,
					Data:    msg.Payload,
				}
...
func (c *Client) SubscribeWithOptionsContext(ctx context.Context, subject string, callback MessageHandler, channelCapacity int) (*Subscription, error) {
	...
	if len(subject) > protocol.MaxSubjectLen {
		return nil, fmt.Errorf("subject too long: %d", len(subject))
	}
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		c.mu.RUnlock()
		if conn == nil {
			err := fmt.Errorf("connection not available")
			lastErr = err
...
func (c *Client) PublishWithContext(ctx context.Context, subject string, data []byte) error {
	...
	if 1+2+len(subject)+len(data) > protocol.MaxBodyLen {
		c.publishErrors.Add(1)
		return fmt.Errorf("message too large: subject=%d payload=%d", len(subject), len(data))
	}
...
func (c *Client) SetMaxReconnectInterval(max time.Duration) {
	c.mu.Lock()
...
func (c *Client) reconnect() {
	...
	go func() {
		...
		for {
			...
			maxBackoff := c.maxReconnectInterval
			c.mu.RUnlock()
			...
			delay := base
			if attempt > 0 {
				delay = base * time.Duration(1<<min(attempt, 10))
			}
			...
			log.Printf("[INFO] Reconnecting to %s in %v...", c.serverAddr, delay)
			time.Sleep(delay)
...
func (c *Client) doReconnect() error {
	...
	oldConn := c.conn
	oldDone := c.done
	key := append([]byte(nil), c.aes256Key...)
	c.mu.RUnlock()
	...
	// 步骤4: 建立新连接
	block, err := newAES256BlockCrypt(key)
	if err != nil {
		return err
	}
	conn, err := kcp.DialWithOptions(c.serverAddr, block, 10, 3)
	if err != nil {
...
func (c *Client) restoreSubscriptions() {
	...
	for i, sub := range subs {
		...
		if err := c.restoreOneSubscription(sub.subject); err != nil {
			log.Printf("[ERROR] Failed to restore subscription %s: %v", sub.subject, err)
			c.subscriptionErrors.Add(1)
			sub.active.Store(false)
			continue
		}
		log.Printf("[INFO] Subscription restored (acked): %s", sub.subject)
	}

	log.Printf("[INFO] All subscriptions restored")
}

func (c *Client) restoreOneSubscription(subject string) error {
	const (
		maxRetries = 3
		ackTimeout = 5 * time.Second
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		err := c.sendSubWithAck(ctx, subject, ackTimeout)
		cancel()
		if err == nil {
			return nil
		}
	...
	return fmt.Errorf("subscription restore failed after %d attempts", maxRetries)
}

func (c *Client) sendSubWithAck(ctx context.Context, subject string, ackTimeout time.Duration) error {
	if len(subject) > protocol.MaxSubjectLen {
		return fmt.Errorf("subject too long: %d", len(subject))
	}
	...
	if _, err := conn.Write(encoded); err != nil {
		...
		delete(c.pendingSubs, subject)
		c.pendingSubsMu.Unlock()
		return fmt.Errorf("failed to send SUB: %w", err)
	}
	...
	case <-time.After(ackTimeout):
		...
		delete(c.pendingSubs, subject)
		c.pendingSubsMu.Unlock()
		return fmt.Errorf("subscription ACK timeout after %v", ackTimeout)
	case <-ctx.Done():
		c.pendingSubsMu.Lock()
		delete(c.pendingSubs, subject)
		c.pendingSubsMu.Unlock()
...
func (c *Client) triggerConnectionLost() {
	...
	if !c.connectionLost.CompareAndSwap(false, true) {
		return
	}
